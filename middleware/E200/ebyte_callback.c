...
void Ebyte_Port_TransmitCallback(uint16e_t state)
{
    ...
    {
        // To-do 实现自己的逻辑
        debug_printfln("TransmitCallback: sendData Success ");
    }
    /* 发送: 异常超时 */
    else if (state & 0x0200)
    {
        // To-do 实现自己的逻辑
        debug_printfln("Ebyte_Port_TransmitCallback: IRQ_RX_TX_TIMEOUT");
    }
    /* 发送: 未知错误 */
    else
    {
        debug_printfln("Ebyte_Port_TransmitCallback: Error");
        /* 发送状态标识不正确，请检查软硬件
...
void Ebyte_Port_ReceiveCallback(uint16e_t state, uint8e_t *buffer, uint8e_t length)
{
    ...
    {
        // To-do 实现自己的逻辑
        debug_printfln("ReceiveCallback: receiveData Success");
    }
    /* 接收: 异常超时 */
    else if (state & 0x0200)
    {
        // To-do 实现自己的逻辑
        debug_printfln("Ebyte_Port_ReceiveCallback: IRQ_RX_TX_TIMEOUT");
    }
    /* 接收: 未知错误 */
    else
    {
        /* 发送状态标识不正确，请检查软硬件
           常见问题 1:SPI通信不正确 2:模块供电不足 */
        debug_printfln("Ebyte_Port_ReceiveCallback:  Error");
        while (1)
            ;
    }
}
